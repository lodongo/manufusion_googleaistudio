...
import type { IndustrySubcategory } from '../../types';
import Modal from '../common/Modal';
import Input from '../Input';
import Button from '../Button';

interface IndustrySubcategoryModalProps {
  isOpen: boolean;
  onClose: () => void;
  onSave: (data: Omit<IndustrySubcategory, 'id'>, id?: string) => Promise<void>;
  subcategory?: IndustrySubcategory | null;
  categoryId?: string;
}

const IndustrySubcategoryModal: React.FC<IndustrySubcategoryModalProps> = ({ isOpen, onClose, onSave, subcategory, categoryId }) => {
  const [formData, setFormData] = useState<Omit<IndustrySubcategory, 'id'>>({ name: '', description: '', enabled: true });
  ...
  const validate = (): boolean => {
    ...
    if (!categoryId) newErrors.form = 'Parent category is missing.';
    setErrors(newErrors);
  ...
  const handleSaveClick = async () => {
    ...
    setIsLoading(true);
    try {
      await onSave(formData, subcategory?.id);
      onClose();
    } catch (e) {
      console.error(e);
...
export default IndustrySubcategoryModal;