...
const ModulesIcon = () => <svg xmlns="http://www.w3.org/2000/svg" className="h-8 w-8 text-white" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M4 6a2 2 0 012-2h2a2 2 0 012 2v2a2 2 0 01-2 2H6a2 2 0 01-2-2V6zM14 6a2 2 0 012-2h2a2 2 0 012 2v2a2 2 0 01-2 2h-2a2 2 0 01-2-2V6zM4 16a2 2 0 012-2h2a2 2 0 012 2v2a2 2 0 01-2 2H6a2 2 0 01-2-2v-2zM14 16a2 2 0 012-2h2a2 2 0 012 2v2a2 2 0 01-2 2h-2a2 2 0 01-2-2v-2z" /></svg>;
...
const OrgIcon = () => <svg xmlns="http://www.w3.org/2000/svg" className="h-8 w-8 text-white" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}><path strokeLinecap="round" strokeLinejoin="round" d="M19 21V5a2 2 0 00-2-2H7a2 2 0 00-2 2v16m14 0h2m-2 0h-5m-9 0H3m2 0h5M5 6h14M5 10h14M5 14h14M5 18h14" /></svg>;
...
const AdminDashboard: React.FC = () => {
    ...
    const [stats, setStats] = useState<{ orgs: number | null, modules: number | null, users: number | null }>({
        orgs: null,
        modules: null,
        users: null,
    });
    ...
    useEffect(() => {
        ...
        const unsubscribeModules = q.onSnapshot((snapshot) => {
            const modulesData = snapshot.docs.map(doc => ({ id: doc.id, ...doc.data() } as Module));
            setModules(modulesData);
            setStats(prev => ({ ...prev, modules: modulesData.length }));
        ...
        const unsubscribeOrgs = db.collection("organisations").onSnapshot(snapshot => {
            setStats(prev => ({ ...prev, orgs: snapshot.size }));
        });
            ...
            <div className="p-4 md:p-8">
                ...
                <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-8">
                    ...
                    <div className="bg-white rounded-lg shadow-md p-6 flex items-center">
                        ...
                        <div>
                            <p className="text-sm text-gray-500 font-medium">Total Organisations</p>
                            {stats.orgs === null ? (
                                <div className="w-6 h-6 border-2 border-blue-200 border-t-blue-500 rounded-full animate-spin mt-2"></div>
                            ) : (
                                <p className="text-3xl font-bold text-gray-800">{stats.orgs}</p>
                            )}
                        </div>
                    </div>
                    {/* Modules Card */}
                    <div className="bg-white rounded-lg shadow-md p-6 flex items-center">
                        <div className="p-4 bg-green-500 rounded-lg mr-4">
                            <ModulesIcon />
                        </div>
                        <div>
                            <p className="text-sm text-gray-500 font-medium">Total Modules</p>
                             {stats.modules === null ? (
                                <div className="w-6 h-6 border-2 border-green-200 border-t-green-500 rounded-full animate-spin mt-2"></div>
                            ) : (
                                <p className="text-3xl font-bold text-gray-800">{stats.modules}</p>
                            )}
                        </div>
                    </div>
                    {/* Users Card */}
                    <div className="bg-white rounded-lg shadow-md p-6 flex items-center">
                        <div className="p-4 bg-indigo-500 rounded-lg mr-4">
                            <UsersIcon />
                        </div>
                        <div>
                            <p className="text-sm text-gray-500 font-medium">Total Users</p>
                            {stats.users === null ? (
                                <div className="w-6 h-6 border-2 border-indigo-200 border-t-indigo-500 rounded-full animate-spin mt-2"></div>
                            ) : (
                ...
                <h2 className="text-2xl font-bold text-gray-800 mb-6">Module Hub</h2>
                <p className="text-gray-600 mb-6">Select a module to view its user-facing dashboard or manage its settings.</p>
                
                {modulesLoading ? (
                     <div className="flex justify-center items-center p-8"><div className="w-10 h-10 border-4 border-blue-500 border-dashed rounded-full animate-spin"></div></div>
                ) : (
