...
interface MasterDataRegistryTabProps {
    currentUser: AppUser;
...
const MasterDataRegistryTab: React.FC<MasterDataRegistryTabProps> = ({ currentUser, theme, organisation, onViewMaterial }) => {
    ...
    useEffect(() => {
        if (currentUser.allocationLevel1Id && currentUser.allocationLevel2Id && currentUser.allocationLevel3Id) {
            ...
            getDocs(query(l4Ref, orderBy('name'))).then(snap => {
                setDepartments(snap.docs.map(d => ({ id: d.id, name: d.data().name })).sort((a,b) => (a.name || '').localeCompare(b.name || '')));
            });
        }
    ...
    useEffect(() => {
        ...
        getDocs(query(l5Ref, orderBy('name'))).then(snap => {
            setSections(snap.docs.map(d => ({ id: d.id, name: d.data().name })).sort((a,b) => (a.name || '').localeCompare(b.name || '')));
        });
...
export default MasterDataRegistryTab;