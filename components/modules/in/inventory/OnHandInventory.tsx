...
interface OnHandInventoryProps {
    currentUser: AppUser;
...
const COLUMNS = [
    ...
    { id: 'bin', label: 'Bin' }, // Added Bin Column
...
const OnHandInventory: React.FC<OnHandInventoryProps> = ({ currentUser, theme, organisation, onViewMaterial }) => {
        ...
        const unsubscribe = onSnapshot(materialsQuery, (snapshot) => {
            ...
            snapshot.forEach(doc => {
                if (doc.ref.path.startsWith(`organisations/${organisation.domain}`)) {
                    ...
                    // Robustly extract Hierarchy IDs. 
                    // If not in data, parse from path: organisations/{org}/level_1/{l1}/level_2/{l2}/level_3/{l3}/level_4/{l4}/level_5/{l5}/materials/{id}
                    const pathParts = doc.ref.path.split('/');
                    // Index 9 is level_4 ID, Index 11 is level_5 ID
    ...
    useEffect(() => {
        const fetchTemplates = async () => {
            ...
            const templatesRef = collection(db, `users/${currentUser.uid}/settings/inventoryTemplates/onHand`);
    ...
    const getValue = (item: any, colId: string) => {
        ...
        switch (colId) {
            ...
            case 'bin': return item.bin || invData.bin || ''; // Check root bin then nested bin
                            ...
                            {filteredData.map((item, idx) => (
                                <tr key={item.id || idx} className="hover:bg-blue-50 transition-colors">
                                    {visibleColumns.map(colId => {
                                        ...
                                        if (['unitCost', 'totalValue'].includes(colId)) displayValue = `$${Number(value).toLocaleString(undefined, {minimumFractionDigits: 2})}`;
