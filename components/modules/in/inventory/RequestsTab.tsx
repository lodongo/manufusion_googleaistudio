...
// Union type to handle both standard material requests and extensions
type RequestItem = (MaterialMasterData & { 
    ...
    materialName?: string; // Explicitly add optional materialName
});

const RequestsTab: React.FC<RequestsTabProps> = ({ currentUser, theme, organisation }) => {
    ...
    useEffect(() => {
        ...
        // Manage two subscriptions
        const unsubMaster = onSnapshot(qMaster, (snap) => {
            updateRequests('master', snap.docs.map(doc => ({ 
                id: doc.id, 
                path: doc.ref.path, 
        ...
        const unsubExt = onSnapshot(qExt, (snap) => {
            updateRequests('ext', snap.docs.map(doc => ({ 
                id: doc.id, 
                path: doc.ref.path, 
                ...doc.data() 
            } as RequestItem)));
        ...
        // Temporary storage to merge updates
        let masterItems: RequestItem[] = [];
        let extItems: RequestItem[] = [];
        
        const updateRequests = (source: 'master' | 'ext', items: RequestItem[]) => {
            if (source === 'master') masterItems = items;
            else extItems = items;
                            ...
                            {filteredRequests.map(req => (
                                <tr key={req.id} className="hover:bg-slate-50">
                                    ...
                                    <td className="px-6 py-4 whitespace-nowrap text-slate-500">
                                        ...
                                        <div className="text-xs">{req.createdAt?.toDate().toLocaleDateString()}</div>
                                    </td>
                                    <td className="px-6 py-4 whitespace-nowrap">
