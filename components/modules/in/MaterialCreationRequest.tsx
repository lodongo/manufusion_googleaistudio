...
import { collection, query, orderBy, getDocs, addDoc, Timestamp, doc, where, onSnapshot, setDoc } from 'firebase/firestore';
...
interface MaterialCreationRequestProps {
  currentUser: AppUser;
...
const MaterialCreationRequest: React.FC<MaterialCreationRequestProps> = ({ currentUser, theme, organisation }) => {
    ...
    const FormField: React.FC<React.ComponentProps<typeof Input>> = ({ label, id, required, ...props }) => {
        return (
          <div className={props.containerClassName}>
            ...
            <Input 
                ...
                required={required} 
                {...props}
                className={`w-full bg-white disabled:bg-slate-100 ${required && !props.value ? 'border-red-400' : 'border-slate-300'} ${props.className || ''}`}
            />
          </div>
        );
    };
...
export default MaterialCreationRequest;
