...
interface NotificationSourceModalProps {
  isOpen: boolean;
  onClose: () => void;
  onSave: (data: NotificationSource) => Promise<void>;
  notificationSource?: NotificationSource | null;
}

const NotificationSourceModal: React.FC<NotificationSourceModalProps> = ({ isOpen, onClose, onSave, notificationSource }) => {
  const [formData, setFormData] = useState<NotificationSource>({ code: '', name: '', description: '', enabled: true });
  const [errors, setErrors] = useState<Record<string, string>>({});
  const [isLoading, setIsLoading] = useState(false);
  const isEditing = !!notificationSource;

  useEffect(() => {
    if (isOpen) {
      if (isEditing && notificationSource) {
        setFormData(notificationSource);
      } else {
        setFormData({ code: '', name: '', description: '', enabled: true });
      }
      setErrors({});
    }
  }, [isOpen, notificationSource, isEditing]);
  ...
  const validate = async (): Promise<boolean> => {
    ...
    if (!isEditing && formData.code) {
        const docRef = db.collection('modules').doc('AM').collection('Sources').doc(formData.code);
  ...
  return (
    <Modal isOpen={isOpen} onClose={onClose} title={isEditing ? 'Edit Notification Source' : 'Add New Notification Source'}>
      <div className="space-y-4">
        ...
        <div className="pt-4 flex justify-end">
          <Button onClick={handleSaveClick} isLoading={isLoading}>
            {isEditing ? 'Save Changes' : 'Create Notification Source'}
          </Button>
        </div>
      </div>
    </Modal>
...
export default NotificationSourceModal;