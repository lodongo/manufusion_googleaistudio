...
import type { MaintenanceDiscipline } from '../../../../types/am_types';
import Button from '../../../Button';
import ConfirmationModal from '../../../common/ConfirmationModal';
import { defaultAmDisciplines } from '../../../../constants/am_disciplines';

const DisciplinesSubTab: React.FC = () => {
    const [disciplines, setDisciplines] = useState<MaintenanceDiscipline[]>([]);
    const [loading, setLoading] = useState(true);
    const [isSeeding, setIsSeeding] = useState(false);
    const [needsSeeding, setNeedsSeeding] = useState(false);
    const [confirmToggle, setConfirmToggle] = useState<MaintenanceDiscipline | null>(null);

    const collectionRef = collection(db, 'modules/AM/disciplines');
    ...
    useEffect(() => {
        ...
        const unsubscribe = onSnapshot(q, (snapshot) => {
            if (snapshot.empty) {
                ...
                setDisciplines(snapshot.docs.map(doc => ({ ...doc.data() } as MaintenanceDiscipline)));
    ...
    const handleSeed = async () => {
        ...
        try {
            ...
            defaultAmDisciplines.forEach(item => {
    ...
    const handleToggle = async (item: MaintenanceDiscipline) => {
        try {
    ...
    if (loading) return <div className="p-8 text-center text-slate-500 italic">Loading disciplines...</div>;

    return (
        <div className="space-y-4">
            <div className="flex justify-between items-center">
                <h3 className="text-sm font-bold text-slate-600 uppercase tracking-wider">Technical Disciplines</h3>
                {needsSeeding && (
            ...
            <ConfirmationModal
                ...
                title={confirmToggle?.enabled ? "Disable Discipline?" : "Enable Discipline?"}
...
export default DisciplinesSubTab;