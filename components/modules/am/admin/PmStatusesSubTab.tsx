...
import type { MaintenanceStatus } from '../../../../types/am_types';
import Button from '../../../Button';
import ConfirmationModal from '../../../common/ConfirmationModal';
import { defaultAmPmStatuses } from '../../../../constants/am_pm_statuses';

const PmStatusesSubTab: React.FC = () => {
    const [statuses, setStatuses] = useState<MaintenanceStatus[]>([]);
    const [loading, setLoading] = useState(true);
    const [isSeeding, setIsSeeding] = useState(false);
    const [needsSeeding, setNeedsSeeding] = useState(false);
    const [confirmToggle, setConfirmToggle] = useState<MaintenanceStatus | null>(null);

    const collectionRef = collection(db, 'modules/AM/status');
    ...
    useEffect(() => {
        ...
        const unsubscribe = onSnapshot(q, (snapshot) => {
            if (snapshot.empty) {
                ...
                setStatuses(snapshot.docs.map(doc => ({ ...doc.data() } as MaintenanceStatus)));
    ...
    const handleSeed = async () => {
        ...
        try {
            ...
            defaultAmPmStatuses.forEach(item => {
    ...
    const handleToggle = async (item: MaintenanceStatus) => {
        try {
    ...
    if (loading) return <div className="p-8 text-center text-slate-500 italic">Loading statuses...</div>;

    return (
        <div className="space-y-4">
            <div className="flex justify-between items-center">
                <h3 className="text-sm font-bold text-slate-600 uppercase tracking-wider">Asset Operational States</h3>
                {needsSeeding && (
            ...
            <ConfirmationModal
                ...
                title={confirmToggle?.enabled ? "Disable Status?" : "Enable Status?"}
...
export default PmStatusesSubTab;