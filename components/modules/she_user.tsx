...
const SheUserPage: React.FC<ModulePageProps> = ({ module, currentUser, onSwitchToAdmin, onBackToDashboard, theme, organisation }) => {
  const canSeeAdminLink = currentUser.accessLevel && currentUser.accessLevel >= 4;
  const [activeTab, setActiveTab] = useState('dashboard');

  const tabs = [
    { id: 'dashboard', label: 'Dashboard' },
    { id: 'incident_reporting', label: 'Incident Reporting' },
    { id: 'risk_assessments', label: 'Risk Assessments' },
    { id: 'safety_inspections', label: 'Safety Inspections' },
  ];
  ...
  const renderTabContent = () => {
    ...
    switch (activeTab) {
      case 'dashboard':
        return placeholderContent('SHE Dashboard', 'This dashboard will provide an overview of safety performance, open incidents, and pending tasks.');
      case 'incident_reporting':
        return placeholderContent('Incident Reporting', 'Users will be able to report new incidents, hazards, and near misses from this section.');
      case 'risk_assessments':
        return placeholderContent('Risk Assessments', 'This section will host the library of Job Hazard Analyses (JHAs) and other risk assessments.');
      case 'safety_inspections':
        return placeholderContent('Safety Inspections', 'Manage and conduct scheduled safety inspections and audits.');
      default:
        return null;
    }
  };
