...
const BudgetGLsTab: React.FC<{ organisation: Organisation; theme: Organisation['theme']; }> = ({ organisation, theme }) => {
    const [structuredCoa, setStructuredCoa] = useState<CategoryWithSubcategories[]>([]);
    const [initialBudgetablePaths, setInitialBudgetablePaths] = useState<Set<string>>(new Set());
    ...
    useEffect(() => {
        ...
        const fetchData = async () => {
            try {
                ...
                setInitialBudgetablePaths(budgetablePaths);
    ...
    const handleSave = async () => {
        setSaving(true);
        const budgetConfigRef = db.doc(`organisations/${organisation.domain}/modules/FI/settings/budgetConfig`);
        const config: BudgetConfig = { budgetableGlAccountPaths: Array.from(selectedAccountPaths) };
        await budgetConfigRef.set(config, { merge: true });
        setInitialBudgetablePaths(new Set(selectedAccountPaths));
        setSaving(false);
        alert('Budgetable GLs saved successfully!');
    };

    const isDirty = useMemo(() => {
        if (initialBudgetablePaths.size !== selectedAccountPaths.size) return true;
        for (const path of initialBudgetablePaths) {
            if (!selectedAccountPaths.has(path)) return true;
        }
        return false;
    }, [initialBudgetablePaths, selectedAccountPaths]);
    ...
    return (
        <div>
            <div className="flex justify-between items-center mb-4">
                <h3 className="text-lg font-medium text-gray-900">Configure Budgetable GL Accounts</h3>
                <Button onClick={handleSave} isLoading={saving} disabled={!isDirty}>Save Changes</Button>
            </div>
             <p className="text-sm text-gray-600 mb-4">Select which of your adopted GL accounts should appear in budget templates.</p>
